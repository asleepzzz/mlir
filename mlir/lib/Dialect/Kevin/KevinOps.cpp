//===- KevinOps.cpp - Kevin MLIR Operations -----------------------------===//
//
...
using namespace mlir::kevin;

//===----------------------------------------------------------------------===//
// KevinDialect Interfaces
//===----------------------------------------------------------------------===//
namespace {
...
KevinDialect::KevinDialect(MLIRContext *context)
    : Dialect(getDialectNamespace(), context) {
  addOperations<
#define GET_OP_LIST
#include "mlir/Dialect/Kevin/KevinOps.cpp.inc"
      >();
}
//KEVIN TEST
...
static LogicalResult verify(Conv2DOp op) {
  ...
  Type operandType2 = op.getOperand(1).getType();
  if (operandType == operandType2) 
      return success();
  else
      return failure();
}
...
static void print(OpAsmPrinter &p, kevin::SubviewOp op) {
  p << op.getOperationName() << "(" << op.getOperands() << ")";
  p.printOptionalAttrDict(op.getAttrs());
  p << " : " << op.getOperands()[0].getType() << " to " << op.getType();
}

static LogicalResult verify(kevin::SubviewOp op) {
  return success();
}
